...
| B.4 `final_at` filled | **met** | Every feed's end field verified live and fixtured (`docs/verification.md`); writer and trigger `games_keep_final_at` (test `042`) stop the wipe; backfilled and read back on hosted: MLB 2,343 of 2,343 finals of 2026 with the four attended exact, MLS 45 exact including Dean's, NFL 7,049 of 7,049; local the same plus 4 of 5 attended NBA (the fifth is the stats path with no wall clock). `mls-ingest.yml` runs `finals.ts` daily; `mlb-sync` and `nba-sync` redeployed with the new core |
| B.5 NFL detail only for logged games | **met** | Migration `20260923000600` (function `games_wanting_detail`, the cleanup) and `run.ts` on demand; local 256 MB to 145 MB after vacuum (`game_appearances` 91 MB to 1.8 MB), hosted 2,793 to 37 detailed games with 48 famous ones now wanting detail for the nightly job; a freshly logged 2025 game detailed by `run.ts --seasons 2025 --queued` alone ("1/285 games", queue settled); `npx tsx ingest/src/verify/relive.ts` all 15 pass; `famous/curated.ts --check` 21 of 21; test `019` |
| B.6 Handshakes in the export | **met** | `export_my_data` rewritten (migration `20260923000400`, on hosted); test `049` exports a handshake both ways, a favorite player, a photo, an MLS shootout result, a block, a notification, preferences and a device token for one user and not the other fan at the same match |
| B.7 Attribution | **met** | `docs/attribution.md` and the About screen carry NBA.com, ESPN and MLS lines in the form of the MLB and nflverse ones, plus the OpenStreetMap, Wikipedia, USGS and Natural Resources Canada sources; the copy test passes |
| C Live state read by the app | **met, unseen live** | `features/live/feeds.ts` (NBA CDN, ESPN MLS), sport-keyed, behind the MLB row shape; `useLiveState` polls with backoff and moves the cached status; game page scoreboard, check-in window, Pick a side lock (`LOCK_RULES` nba live, mls new) and the eggs (`EGG_SPORTS` nba on, mls new) all read it; `feeds.test.ts` 10, lock tests for both, core `parseMlsLiveState`; VERIFY from the device build: `docs/evidence/live/probe-feeds-from-device-build.png` (CDN 200 with the nba.com header set, ESPN 200 with React Native's own User-Agent). No game was under way to screenshot; CLAUDE.md's ground rule amended |
| D.2 Players seen, for good games | **met** | `goodGame.ts` with a test per threshold; lines parsed from the MLB box (Greene 2 HR 3 RBI on the 2024-09-15 fixture), the NBA box and nflverse's weekly stats; migration `20260923000700` (columns, `players_seen`, `players_seen_count`, `game_players_seen`), test `020` (a superstar's one good game counts, a role player's eleven quiet games do not, a ten-timer earns a row); local attended games re-detailed (MLB, NBA, 88 NFL); screenshots per sport in `docs/evidence/players-seen/`; on hosted with Dean's MLB games re-detailed |
| E.1 MLS Elo with draws | **met** | `runEloThreeWay` (Davidson draw term) with tests; `sweep.ts --sport mls` over 720 combinations, three-way log loss 1.0358 vs 1.0518 constant (`docs/elo-backtest.md`); `run.ts --sport mls` wrote 30 ratings and 4,962 probabilities on local and on hosted, read back (average draw 0.273) |
| E.2 MLS Pick a side, draw voids | **met** | Migration `20260923000800`: `make_pledge` allows MLS with the side's own three-way probability, `validate_pledges_for_game` voids a level match (`draw`), shootout included, extra time scores, "Drawn, no result" notification; `game_context` carries `draw_prob`; test `021` (12 assertions: the four decision cases); the app's MLS exclusions on the check-in screen are gone, the pills and cards say Drawn; core `LOCK_RULES.mls` on. Not seen live: no MLS match was under way and check-in needs a location |
| E.3 MLS rosters and favorite players | **met** | `MlsProvider.fetchRoster` and `parseMlsRoster` (fixture test on Inter Miami's roster); `ingest/src/mls/rosters.ts`: 945 rows across 30 clubs on local and hosted, read back; the MLS exclusions in the league picker, the roster page, the add-team prompt and onboarding are gone (the onboarding test now expects the Union's section); simulator: `docs/evidence/mls/journey/players-prompt-inter-miami.png` |
| D.1 NBA and MLS superstars | **met** | `seed/nba_awards.json` (286 rows, 2013-14 on) and `seed/mls_awards.json` (176 rows, 2016 on), sources on every row (docs/verification.md: the NBA MVP top five is not on Wikipedia, so pr.nba.com releases; every MLS id checked in a lineup); loaders `ingest/src/nba/honors.ts`, `ingest/src/mls/honors.ts` through the shared `famous/awards.ts`; migration `20260923001000` (honor kinds), test `023`; `franchise.ts` resolves NBA (commonallplayers) and MLS (ESPN search, ids for the 21 names it knows twice) and the draft is merged: 350 franchise players on local and hosted, 286 + 176 honor rows read back on hosted; simulator: `docs/evidence/superstars/nba-game-0021600001-players-seen.png` (LeBron James, Finals MVP 2015, with his triple-double) |
| G.1 Light mode | **met** | Every reachable screen walked in the simulator in light mode under the Phillies theme and the Bears theme, 50 routes each, by script (`docs/evidence/light/sheet-phillies-light.png`, `sheet-bears-light.png`, the full-size shots beside them); a dev link `?themeTeam=<id>` picks the theme the app wears so a walk can be scripted; nothing that was readable in dark is invisible in light (chips, text fields inside cards, toggles, badges and rings all checked); accents follow the chosen team (red buttons and links under the Phillies, navy under the Bears) |
| G.2 Signed-out screens | **met** | email, code and the six onboarding steps, both modes, as a fresh confirmed user (`docs/evidence/signed-out/sheet-signed-out.png`); the name field's placeholder rendered letter-spaced ("D e a n") on that launch in both modes and normally on a later launch, so it is transient, not a style |
| G.3 MLS journey | **met** | `docs/evidence/mls/journey/`: the players prompt (E.3), the game page for the Inter Miami draw and the 2022 MLS Cup shootout, search, the log sheet, the stamp (Nu Stadium), the MLS bucket list (1 of 30), players seen and Messi's page, Relive, Wrapped and the share card, all in light mode; the check-in card says match, not game |
| G.4 Parity | **met** | `npm run parity` on 2026-09-22 after the light-mode walk: 34 shots compared (17 screens, both modes), mean 6.55% mismatch, worst relive-mid light 21.03%, passport-all light 4.14% (the same figure as the 2026-09-16 run); sheets left in `design/parity/sheets/` for Dean to approve, not self-certified |
| F Palettes | **met** | `seed/scripts/compare_team_colors.py`: all 125 palettes against jimniels/teamcolors (no licence, 22 of 30 MLS clubs, dated) and ESPN's current colour pair; the full table in `docs/palette-diff.md`, the 30 changed rows and the reasoning in `docs/verification.md`; the defect found was every MLS `--t2` repeating its primary; contrast check passes; applied to the seed, local and hosted; `docs/evidence/palettes/before-after.png` for the ten biggest |
| E.4 MLS detail and Relive | **met** | `packages/core/src/providers/mls/detail.ts` (parse the summary into appearances with lines, the goal timeline, plays, moments, the pledge lock, the end time; the story and the state-model line; 47 tests on the 2022 MLS Cup and the 2026 Inter Miami draw), `ingest/src/mls/detail.ts` (`--events`, `--queue`, `--attended`), daily `--queue` in `mls-ingest.yml`; `ingest/src/verify/relive.ts` checks 5 MLS matches against the scoreboard's `details[]` (all 20 games across four sports check out); the Relive route and the game page are ungated, "Kick-off / Halftime / Full time", highlights open the MLS video hub; simulator: `docs/evidence/mls/relive/` (pregame, halftime 1-1, full time 2-2) |
| E.5 MLS Wrapped | **met** | Migration `20260923000900` (the daily job publishes MLS a month back; MLS moments ranked), test `022` (7 assertions); the app lists MLS seasons and says matches; `docs/evidence/mls/wrapped/wrapped-mls-2026-preview.png`; the game page's "not available yet" sentence is gone |
| ESPN venue ids are per sport (found under B.1) | **met** | 17 New England home matches were in Paris on hosted; migration `20260923000300` splits soccer ids into `espn_soccer_venue_ids` and moves them back; read back: 0 at Accor Arena, 17 of 2026 at Gillette; test `048` |
...
- **EAS project slug is `jinx`** (id `ea474a72-1186-4600-90e1-8dffcdbcafa2`, owner deanyao), declared in `apps/mobile/app.json`, which is where every `eas` and `expo` command runs from. An earlier `eas init` at the repo root had made a second project (`7746f5f4-...`) and left a root `app.json` naming it; that file was deleted on 2026-09-23 because `eas update` run from the root would have published to the abandoned project instead of the one the builds come from.
