 ...
 */
import { NbaClient, modelHomeWp, parseEspnScoreboard, clockToSeconds, secondsLeftInGame } from '@jinx/core';
...
for (const ym of ['201811', '201812', '201901', '201902']) {
  ...
  const infos = parseEspnScoreboard(doc.events ?? []).filter((i) => i.final && i.homeScore != null && i.awayScore != null);
  for (const info of infos.slice(0, 40)) {
    ...
    for (const p of s.plays ?? []) {
      ...
      const e = wp.get(p.id);
      if (typeof e === 'number') { const c = Math.min(Math.max(e, 1e-4), 1 - 1e-4); espnLoss.push(won ? -Math.log(c) : -Math.log(1 - c)); }
    }
    games++;
  }
}
console.log(games, 'games,', samples.length, 'play states; ESPN log loss', (espnLoss.reduce((a, b) => a + b, 0) / espnLoss.length).toFixed(4));
for (const scale of [0.08, 0.10, 0.12, 0.14, 0.16, 0.18, 0.20, 0.24]) {
  let sum = 0;
  for (const s of samples) { const p = Math.min(Math.max(modelHomeWp(s.margin, s.left, 0.5, { marginScale: scale, priorWeight: 1 }), 1e-4), 1 - 1e-4); sum += s.won ? -Math.log(p) : -Math.log(1 - p); }
