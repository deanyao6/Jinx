...
async function main(): Promise<void> {
  ...
  const only = new Set(arg('teams')?.split(',').map((s) => s.trim()).filter(Boolean) ?? []);
  const teams = (
    await selectAll<TeamRow>(db, 'teams', 'id, provider_team_id, abbreviation', (q) => q.eq('provider', 'nba').eq('active', true))
  ...
  for (const team of teams.sort((a, b) => a.abbreviation.localeCompare(b.abbreviation))) {
    try {
      ...
      const r = await upsertTeamRoster(db, { provider: 'nba', sport: 'nba', teamId: team.id, season, entries });
      total += r.written;
      console.log(`${team.abbreviation}: ${r.written} on roster (${r.newPlayers} new players, ${r.removed} removed)`);
    } catch (err) {
      failures += 1;
