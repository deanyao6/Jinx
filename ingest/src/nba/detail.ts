...
async function pendingIds(
  db: Db,
  limit: number,
): Promise<{ id: string; provider_game_id: string }[]> {
...
async function main(): Promise<void> {
  ...
  if (process.argv.includes('--queue')) {
    const result = await drainNbaQueue(db, provider, provider.client, ctx, {
      detailLimit: limit,
      reliveLimit: limit,
    });
  ...
  if (ids?.length) {
    const { data } = await db
      ...
      .in('provider_game_id', ids);
    const known = new Map(
      ((data ?? []) as { id: string; provider_game_id: string }[]).map((r) => [
        r.provider_game_id,
        r.id,
      ]),
    );
  ...
  for (const t of targets) {
    ...
    if (
      ...
      console.log(
        `${t.provider_game_id}: skipped (unknown team ${detail.homeProviderTeamId} / ${detail.awayProviderTeamId})`,
      );
      continue;
    }
