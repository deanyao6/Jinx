...
export function LiveSessionCard({ gameId }: { gameId: string }) {
  ...
  const live = useLiveState(
    gameId,
    c?.sport_id,
    !!c && hasLiveFeed(c.sport_id) && c.status !== 'final',
  );
  ...
  const pick = c.pledge
    ? `Pick: ${c.pledge.team_id === c.home.team_id ? c.home.name : c.away.name}. `
    : '';
  // In the colours of the side this person is on, and the home team's until there is one.
  const side = c.attendance?.rooting_team_id ?? c.pledge?.team_id ?? c.home.team_id;
  return (
    <SideTheme team={side}>
...
export function checkInOffers(input: {
  ...
  for (const g of input.today) {
    ...
    if (input.loggedIds.has(g.id)) out.push({ game: g, reason: 'logged' });
      ...
      if (
        v &&
        distanceMeters(input.here.lat, input.here.lng, v.lat, v.lng) <=
          v.geofence_m + Math.min(input.here.accuracyM, 200)
      ) {
...
function CheckInOffer({ exceptGameId }: { exceptGameId: string | null }) {
  ...
  const today = useMemo(
    () =>
      (games.data ?? []).filter(
        (g) => g.id !== exceptGameId && isTodayAtVenue(g.scheduled_start, g.venue?.tz, mountedAt),
      ),
  ...
  const ticketIds = useMemo(
    () =>
      new Set(
        (imports.data ?? [])
          .map((i) => (i as { matched_game_id?: string | null }).matched_game_id)
      ...
      try {
        ...
        if (alive)
          setHere({
            ...
            accuracyM: pos.coords.accuracy ?? 500,
          });
      } catch {
  ...
  const venues = useMemo(
    () => new Map<string, { lat: number; lng: number; geofence_m: number }>(),
    [],
  );
