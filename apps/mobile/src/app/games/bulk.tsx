...
export default function BulkLogScreen() {
  ...
  return (
    <Screen>
      <Text variant="h1">Log a season</Text>
      <Text color="muted" style={{ marginBottom: theme.spacing.md }}>
        Pick a team and season, then check every game you went to. Games already on your passport
        are skipped.
      </Text>
      ...
      {!team ? (
        <EmptyState title="Pick a team to start" />
      ) : games.isPending ? (
        <Loading label={`Loading ${season} games`} />
      ) : list.length === 0 ? (
        <EmptyState title="No games" body={`No ${season} games for the ${team.name} yet.`} />
      ) : (
        <Card label={`${list.length} games · ${selected.size} selected`}>
          <View
          ...
          >
            ...
            />
            <Button title="Clear" variant="ghost" small onPress={() => setSelected(new Set())} />
          </View>
          {list.map((g, i) => {
            const home = byId.get(g.home_team_id);
            const away = byId.get(g.away_team_id);
