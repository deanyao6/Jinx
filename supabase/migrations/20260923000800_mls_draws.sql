-- The MLS second wave, part one (next-wave E.1 and E.2, Dean's decision 14): a draw-aware model,
-- and Pick a side at a match with a draw voiding the pledge.
--
-- game_win_prob gains the draw: for MLS `home_win_prob` is P(home wins), `draw_prob` P(draw), and
-- the away side is the rest, from the three-outcome Elo in packages/core/src/elo.ts
-- (method elo_draw_v1). The other sports keep draw_prob null.
alter table public.game_win_prob add column if not exists draw_prob numeric(6,5);
alter table public.game_win_prob drop constraint if exists game_win_prob_draw_prob_check;
alter table public.game_win_prob add constraint game_win_prob_draw_prob_check
  check (draw_prob is null or (draw_prob >= 0 and draw_prob <= 1 and draw_prob + home_win_prob <= 1));

-- The estimate without live state: kick-offs run about 13 minutes late, and the lock is the
-- first goal or halftime, so a quarter of an hour (features/checkin/lock.ts has the same row).
...
as $$
  select case
    when p_sport = 'nfl' then p_start + interval '12 minutes'
    when p_sport = 'mls' then p_start + interval '15 minutes'
    else p_start + interval '30 minutes'
  end;
$$;

-- make_pledge: MLS is allowed, and the pledged side's probability is its own three-way number,
-- never "one minus the other side" (that would count the draw for it).
...
begin
  ...
  select home_win_prob, draw_prob into v_prob, v_draw from public.game_win_prob where game_id = p_game_id;
  v_team_prob := case
    when v_prob is null then (case when g.sport_id = 'mls' then 0.375 else 0.5 end)
    when p_team_id = g.home_team_id then v_prob
    else 1 - v_prob - coalesce(v_draw, 0) end;
...
-- Validation: an MLS match that ends level on goals voids the pledge (reason `draw`): not a win,
-- not a loss, not scored, out of every count. A shootout does not change that (the match was a
-- draw; the penalties decided who advances), and neither does the aggregate; extra time is
-- just more of the match, so a winner after it scores normally. Other sports keep `tie`.
...
begin
  ...
  for p in select * from public.pledges where game_id = p_game_id and status = 'provisional' loop
    ...
    if g.sport_id = 'mls' and v_result = 'tie' and v_status = 'valid' then
      v_status := 'void'; v_reason := 'draw'; v_result := null;
    end if;
    ...
    elsif v_reason = 'draw' then
      insert into public.notifications (user_id, kind, title, body, data)
      values (p.user_id, 'pledge_void', 'Drawn, no result',
              format('%s drew, so your pick does not count either way.', v_team_name),
              jsonb_build_object('game_id', p_game_id, 'reason', 'draw'));
    else
      insert into public.notifications (user_id, kind, title, body, data)
...
-- game_context: each side's own probability (the away side never inherits the draw), and the
-- draw itself, so Pick a side can say "Draw 27%".
...
begin
  ...
  return jsonb_build_object(
    ...
    'away', jsonb_build_object('team_id', g.away_team_id, 'name', g.away_name, 'favorite', v_away_fav, 'win_prob', case when v_prob is null then null else 1 - v_prob - coalesce(v_draw, 0) end),
    'draw_prob', v_draw,
